...
- token 검증 결과는 신뢰하되, 내부 계정 연결은 `(provider=KEYCLOAK, provider_subject=sub)` 기준으로만 수행합니다.
- email은 자동 계정 연결 키로 쓰지 않습니다. 동일 email의 기존 사용자가 있으면 `KeycloakAccountConflictException(AUTH-004, HTTP 409)`을 반환합니다.
